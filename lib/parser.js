...
Parser.prototype = {
  ...
  attrs: function () {
    ...
    while (this.peek().type == 'attrs' || this.peek().type == 'id' || this.peek().type == 'class') {
      switch (this.peek().type) {
      case 'attrs':
        attrs.push(attributes(this.peek().value));
        break;
      case 'id':
...
function attributes(str) {
  ...
  while (str.length) {
    ...
    var captures;
    if (match) {
      captures = /["|'](.*)["|']/.exec(object[key]);
      object[key] = captures[1];
    } else {
      if (captures = /(.*)!/.exec(object[key])) {
        object[key] = locals(captures[1]);
      } else {
        object[key] = variable(object[key]);
      }
    }
