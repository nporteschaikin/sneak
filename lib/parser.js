...
Parser.prototype = {
  ...
  js: function () {
    var js = [];
    js.push("with (locals || {}) {")
    js.push("  property = function(key) {");
    js.push("    if (locals[key] !== undefined) {");
    js.push("      return locals[key]");
    js.push("    } else {");
    js.push("      throw new ReferenceError(key + 'is not defined')");
    js.push("    }");
    js.push("  }");
    js.push("  var buffer = [");
    js.push('    "' + this.buf.join('","') + '"');
    js.push("  ];");
    js.push("  return buffer.join('');");
    js.push("}")
    return js.join("\n");
  },
